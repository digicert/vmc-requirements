...
The five types of Mark Certificates included in these MC Requirements are as follows:
...
|1.7 |01-22-2025 |01-22-2025
|1.8 |0X-XX-2026 |0X-XX-2026
|===
...
*Certificate Requester*: A natural person who is either the Applicant, employed by the Applicant, an authorized agent who has express authority to represent the Applicant, or a third party (such as an ISP or hosting company) that completes and submits a Mark Certificate Request on behalf of the Applicant.
...
*MC Authority:* A source other than the Certificate Approver, through which verification occurs that the Certificate Approver is expressly authorized by the Applicant, as of the date of the Mark Certificate Request, to take the Request actions described in these Requirements.

*Mark Certificate Request:* A request from an Applicant to the CA requesting that the CA issue a Mark Certificate (MC) to the Applicant, which request is validly authorized by the Applicant and signed by the Certificate Approver.
...
*Provisional Mark Certificate:* One type of Common Mark Certificate.
...
*Registered Mark:* A mark that has been registered by at least one Trademark Office.
...
The Static Certificate Transparency API, v1.1.0, https://c2sp.org/static-ct-api@v1.1.0
...
. Modified Registered Marks MAY be shown in any font or color against a colored or patterned background.
...
===== Provisional Mark Verification

====== Verification of Provisional Mark
The CA will perform the verification steps under <<_verification_of_mark_with_trademark_office>> and <<_verification_of_registered_mark_ownership_or_license>> for a Registered Mark submitted by the Applicant. Additionally, the Applicant will provide a Mark Representation that the Applicant wishes to include in the Mark Certificate. The representation of the Registered Mark and the Mark Representation that the Applicant wishes to include in the Mark Certificate MAY differ.

The CA SHALL verify that the Registered Mark was registered by the Trademark Office at least three years prior to the date of the submission of the Mark Certificate Request for the Mark Certificate.
...
. For Word Marks that consist of a single word, the one word MAY be separated into multiple parts which may be stacked or not.
. For Word Marks that consist of multiple words, the words MAY be separated into multiple parts which may be stacked or not, or the multiple words MAY be combined into a single word.

The following are examples of permitted modifications to registered Word Marks.
...
+
Mark Certificates pre-certificates MUST be logged to one or more Certificate Transparency (CT) logs [RFC6962] which then provide one or more Signed Certificate Timestamps (SCT). The SCT(s) must be added to the Certificate Transparency extension as a SignedCertificateTimestampList encoded as an octet string [RFC6962 section 3.3]. The AuthIndicators Working Group maintains a list of acceptable CT logs, and the current list is attached as <<_ct_logs_approved_by_authindicators_working_group>>. This extension is REQUIRED, and SHOULD NOT be marked critical.
...
+
*Required* if the subject:markType field contains one of the following values: “Registered Mark”, “Modified Registered Mark”, or “Provisional Mark”; *Prohibited* otherwise
+
*Contents:* This field MUST contain the country or region of the Trademark Office that registered the Registered Mark as a WIPO ST.3 two letter country and intergovernmental/regional agency code (see list at http://www.wipo.int/export/sites/www/standards/en/pdf/03-03-01.pdf).

. *Certificate field:* subject:trademarkOfficeName (OID: 1.3.6.1.4.1.53087.1.2)
+
*Required* if the subject:markType field contains one of the following values: “Registered Mark”, “Modified Registered Mark”, or “Provisional Mark” and the applicable country/region has more than one national/regional intellectual property agency where trademarks can be registered; *Optional* if the subject:markType otherwise contains one of the following values: “Registered Mark”, “Modified Registered Mark”, or “Provisional Mark”; *Prohibited* if the subject:markType does not contain one of the following values: “Registered Mark”, “Modified Registered Mark”, or “Provisional Mark”
+
*Contents:* This string value identifies the Trademark Office by inserting the Trademark Office name listed in the “Office” column in the WIPO directory of country and regional intellectual property agencies at https://www.wipo.int/directory/en/urls.jsp for the Trademark Office that registered the Registered Mark that was submitted by the Applicant.
...
[%header,cols="1,1,1"]
|===
|Section number of Mark validation method |subject:markType value| Certificate type

|<<_registered_mark_verification>> |Registered Mark| Verified Mark Certificate
|<<_government_mark_verification>> |Government Mark| Verified Mark Certificate
|<<_proof_of_prior_use>> |Prior Use Mark| Common Mark Certificate
|<<_modification_of_registered_trademark>> |Modified Registered Mark| Common Mark Certificate
|<<_provisional_mark_verification>> |Provisional Mark| Common Mark Certificate
|===
...
+
If the subject:markType field contains “Prior Use Mark”, then this field is *Required*. Otherwise, it is *Prohibited*.
+
...
This Appendix lists the Certificate Transparency logs that are acceptable for logging Pre-certificates to comply with these Requirements. The listed log(s) conform to either [RFC6962] or https://c2sp.org/static-ct-api@v1.1.0[static-ct-api version 1.1.0].

[cols=",,",]
|===
|Log URL |Name |Kind
|https://gorgon.ct.digicert.com/log|Gorgon |RFC 6962
|===
